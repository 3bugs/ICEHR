...
function doAddCourse()
{
  ...
  if ($trainerId == null) {
    $sql = "INSERT INTO course (course_master_id, batch_number, details, application_fee, trainee_limit, place, place_type, begin_date, end_date, responsible_user_id, is_online) "
        . " VALUES ($courseMasterId, $batchNumber, '$details', $applicationFee, $traineeLimit, '$place', $placeType, '$beginDate', '$endDate', $responsibleUserId, $isOnline)";
  } else {
...
function doUpdateCourse()
{
  ...
  if ($trainerId == null) {
    $sql = "UPDATE course SET course_master_id = $courseMasterId, batch_number = $batchNumber, details = '$details', application_fee = $applicationFee, "
        . " trainee_limit = $traineeLimit, place = '$place', place_type = $placeType, begin_date = '$beginDate', end_date = '$endDate', responsible_user_id = $responsibleUserId, is_online = $isOnline "
        . " WHERE id = $courseId";
  } else {
    $sql = "UPDATE course SET course_master_id = $courseMasterId, batch_number = $batchNumber, details = '$details', application_fee = $applicationFee, "
...
function doAddNews()
{
  ...
  if ($result = $db->query($sql)) {
    ...
    for ($i = 0; $i < sizeof($_FILES[KEY_PDF_FILES]['name']); $i++) {
      if ($_FILES[KEY_PDF_FILES]['name'][$i] !== '') {
        $fileName = null;
...
function doUpdateNews()
{
  ...
  /*if ($_FILES['coverImageFile']) {
        $response[KEY_ERROR_CODE] = ERROR_CODE_ERROR;
        $response[KEY_ERROR_MESSAGE] = print_r($_FILES, true);
        $response[KEY_ERROR_MESSAGE_MORE] = '';
        return;
    }*/

  $coverImageFileName = NULL;
  if ($_FILES['coverImageFile']['name'] !== '' /*$_FILES['coverImageFile']*/) {
    if (!moveUploadedFile('coverImageFile', UPLOAD_DIR_NEWS_ASSETS, $coverImageFileName)) {
      $response[KEY_ERROR_CODE] = ERROR_CODE_ERROR;
      $response[KEY_ERROR_MESSAGE] = 'เกิดข้อผิดพลาดในการอัพโหลดไฟล์ (รูปภาพ)';
      $response[KEY_ERROR_MESSAGE_MORE] = '';
